...
echo -e "${GREEN}----------------------------------------------------${NC}"
# 'mesa' para GPUs AMD/Intel [1]
# 'git' para clonar repositórios (se necessário, embora priorizemos pacman) [1]
# 'libinput' para gerenciamento de entrada [2]
# 'wayland-protocols' para definições de protocolo Wayland [3]
...
echo -e "${GREEN}----------------------------------------------------${NC}"
# 'hyprland': O compositor Wayland [4]
# 'hyprpaper': Utilitário de papel de parede nativo do Hyprland [5]
# 'xdg-desktop-portal-hyprland': Essencial para compartilhamento de tela e portais XDG [6, 7]
# 'xdg-desktop-portal-gtk': Recomendado para diálogos de arquivos [6]
...
confirm_action "Deseja instalar a barra de status Waybar? (Recomendado para Hyprland) [10]"
if$ ]]; then
    ...
    echo -e "${YELLOW}E de ajustar as referências de 'sway/workspaces' para 'hyprland/workspaces' em sua configuração.${NC}" [10]
fi
echo

# Lançadores de Aplicações (Apenas Fuzzel)
confirm_action "Deseja instalar o lançador de aplicações Fuzzel? (Recomendado) [11]"
if$ ]]; then
...
confirm_action "Deseja instalar o gerenciador de arquivos Thunar? (Leve e funcional) [12]"
if$ ]]; then
...
confirm_action "Deseja instalar o daemon de notificação Mako? (Leve e projetado para Wayland) [13, 14]"
if$ ]]; then
...
confirm_action "Deseja instalar ferramentas de captura de tela nativas para Wayland (grim, slurp, grimshot)? (Recomendado) [15, 16, 17]"
if$ ]]; then
...
if$ ]]; then
    ...
    systemctl --user enable pipewire.service [18]
    systemctl --user enable pipewire-pulse.service [18]
    systemctl --user enable wireplumber.service [18]
...
echo -e "${GREEN}----------------------------------------------------${NC}"
echo -e "${YELLOW}O utilitário 'hyprpaper' já foi instalado com o Hyprland e é suficiente para definir papéis de parede.${NC}" [4]
echo -e "${YELLOW}Você pode configurá-lo no seu arquivo ~/.config/hypr/hyprland.conf.${NC}"
echo

# Gerenciamento de Rede (NetworkManager)
confirm_action "Deseja instalar o NetworkManager e seu applet gráfico? (Altamente recomendado para gerenciar conexões de rede) [19, 20]"
if$ ]]; then
    install_packages networkmanager network-manager-applet
    echo -e "${GREEN}Ativando o serviço NetworkManager...${NC}"
    sudo systemctl enable --now NetworkManager.service [20]
    if [ $? -ne 0 ]; then
...
echo -e "${YELLOW}   ${NC}exec-once=dbus-update-activation-environment --systemd WAYLAND_DISPLAY XDG_CURRENT_DESKTOP" [7]
echo -e "${YELLOW}   ${NC}exec-once=waybar" # Para iniciar a barra de status [10]
echo -e "${YELLOW}   ${NC}exec-once=mako" # Para iniciar o daemon de notificações [13]
echo -e "${YELLOW}   ${NC}exec-once=nm-applet --indicator" # Para o ícone do NetworkManager na barra [20]
echo -e "${YELLOW}   ${NC}exec-once=hyprpaper" # Para definir o papel de parede [4]
...
echo -e "${YELLOW}   Lembre-se de substituir as ocorrências de ${NC}'sway/workspaces'${YELLOW} por ${NC}'hyprland/workspaces'${YELLOW} na configuração do Waybar para compatibilidade total.${NC}" [10]
echo
...
echo -e "${YELLOW}Após reiniciar, se você instalou o SDDM, selecione 'Hyprland' no seu Display Manager na tela de login. Caso contrário, inicie-o manualmente de um TTY (Ctrl+Alt+F2) digitando 'Hyprland'.${NC}" [1]
echo
echo -e "${GREEN}====================================================${NC}"
